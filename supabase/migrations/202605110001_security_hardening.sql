-- FEG Stage PRO 3.7.4 - Supabase migration
-- Apply with: supabase db push
-- Existing installations: backfill public.projects.owner_id before strict RLS.
-- See supabase/manual/owner_id_backfill_template.sql for a safe manual template.
...
to authenticated
using (public.feg_project_role(id) in ('owner', 'editor', 'viewer'));
...
to authenticated
using (public.feg_project_role(id) in ('owner', 'editor'))
with check (public.feg_project_role(id) in ('owner', 'editor'));
...
to authenticated
with check (public.feg_project_role(project_id) = 'owner');

create policy "feg_project_memberships_update_owner"
on public.project_memberships for update
to authenticated
using (public.feg_project_role(project_id) = 'owner')
with check (public.feg_project_role(project_id) = 'owner');
